...
def Register(self, project_name, suffix_query, notif_method, template, counter_atk, will_expire_in):
    '''
    ...
    for each_url_suffix in url_suffix_list:
        ...
        if db.Exist(each_url_suffix, db_name, tbl_name):
...
def Delete(self, suffix_query):
    ...
    for each_url_suffix in _Suffix_query_parse(suffix_query):
        ...
        if not db.Exist(each_url_suffix, db_name, tbl_name):
            continue
        # Remove the record from database's table
        db.Remove(each_url_suffix, db_name, tbl_name)
